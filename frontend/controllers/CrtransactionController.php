...
use frontend\models\CrtransactionSearch;
 ...
 */
class CrtransactionController extends Controller
{
    /*  public $val=7;
    public $order_num=7;*/
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['create', 'index', 'update', 'view', 'delete', 'ask', 'report', 'long-report', 'ask-shift', 'shift-report','error'],
                        'allow'   => true,
                        'roles'   => ['@'],
                    ],
                ],
    ...
    public function actionIndex()
    {
        ...
        $this->layout                       = "cafeRestLayout";
        $searchModel                        = new CrtransactionSearch();
    ...
    public function actionCreate()
    {
        ...
        $this->layout = 'cafeRestTranLayout';
        ...
        if ($order_table->load(Yii::$app->request->post())) {
            $tables    = $connection->createCommand('SELECT table_state from rtable_session where table_id=' . $order_table->order_tabe)->queryOne();
            $date_time = date('Y-m-d H:i:s');
            if ($tables["table_state"] == 0) {
                // UPDATE table_name SET column1=value1,column2=value2,...WHERE some_column=some_value;
                $update_session = $connection->createCommand('UPDATE rtable_session set table_state=1,session_start="' . $date_time . '" where table_id = ' . $order_table->order_tabe);
        ...
        $model = new Session();
        // $table=new Tables();
        $modelsTransaction = [new Crtransaction];

        if ($model->load(Yii::$app->request->post())) {
            //$model->date = date('Y/m/d');
            // $model->save();

            $modelsTransaction = Model::createMultiple(Crtransaction::classname());
                ...
                try {
                    ...
                    if ($flag = $model->save(false)) {
                        foreach ($modelsTransaction as $modelTransaction) {
                           ...
                           // $modelTransaction->order_status= 'o';
        ...
        'modelsTransaction' => (empty($modelsTransaction)) ? [new Transaction] : $modelsTransaction,
            ...
            return $this->render('create', [
                ...
                'invoic_table'      => $invoic_table,
                'order_table'       => $order_table,
                'modelsTransaction' => (empty($modelsTransaction)) ? [new Crtransaction] : $modelsTransaction,
            ]);
        }
    }
    ...
    {
        if (($model = Crtransaction::findOne($id)) !== null) {
            return $model;
        } else {
