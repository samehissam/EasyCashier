           ...
           if ($invoic_table->load(Yii::$app->request->post()))
                {
                    ...
                    $update_session = $connection2->createCommand('UPDATE rtable_session set table_state=0  where table_id = '.$invoic_table->table_id );
                    $update_session->execute();

                }
                $tables = $connection->createCommand('SELECT * from rtable_session')->queryAll();
                $e=0;
...
<?php
    date_default_timezone_set('Africa/Cairo');
        ...
        if ($invoic_table->load(Yii::$app->request->post()))
        {
        $table_session_start = $connection->createCommand("SELECT session_start FROM rtable_session where table_id=".$invoic_table->table_id )->queryOne();
